//p_write.c
//writes a random pattern to areas of memory
//Tyler Maydew
//GCC compiler
//Created 10/1/2018
//ECEN 5813
...
uint8_t p_write(uint32_t **mem_block, uint64_t requested_address, uint32_t num_bytes, uint32_t num_bytes_to_display, uint32_t seed, int8_t easy_addressing){
    //clock measurement
    ...
    if(!easy_addressing){
        ...
        check_greater = (requested_address > ((uint64_t) (&(*mem_block)+(num_bytes)) - (uint64_t) (&(*mem_block))));
